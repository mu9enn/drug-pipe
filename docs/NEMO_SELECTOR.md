...
都计入分母和保留数；不拆同分样本、不改变比较边界，不额外抽样凑数。
2026-09-14 起，20%～30% 仅为优先目标：不可达时选择最接近 25% 的实际可达值，报告偏差并继续交付，
不因比例中止训练衔接；数据完整性、原生格式和训练长度检查仍必须通过。
...
不修改奖励、读取顺序、生成预算。完整教师回答仍参与现有 32K／64K 分档，超长记录单列而不删除。
用户已于 2026-09-14 授权验证完成后自动提交 8 卡 RL。恢复流程直接检查已完成的官方相似度缓存，
不等待旧 worker 的退出标记；阈值、导出、读取验证和提交各有日志及超时。提交成功必须由调度器中
实际存在且命令匹配的任务对象确认，不能仅凭提交命令退出码判断。GPU 入口运行有总时限的真实前台训练。
