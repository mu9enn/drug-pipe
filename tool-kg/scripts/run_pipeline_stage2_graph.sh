...
if [[ "$RUN_ID" == "--help" || "$RUN_ID" == "-h" ]]; then
  echo "Usage: $0 <run_id> [--alert-rerun] [--max-alert-rerun-rounds <n>] [--max-workers <n>] [--harness claude|deepseek] [--resume]"
  exit 0
fi

if [[ -z "$RUN_ID" ]]; then
  echo "Usage: $0 <run_id> [--alert-rerun] [--max-alert-rerun-rounds <n>] [--max-workers <n>] [--harness claude|deepseek] [--resume]" >&2
  exit 1
fi
shift || true
