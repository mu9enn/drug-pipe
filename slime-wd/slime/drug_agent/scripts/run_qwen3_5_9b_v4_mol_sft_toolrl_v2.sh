...
EXPECTED_TOOLRL_PROTOCOL="${EXPECTED_TOOLRL_PROTOCOL:-toolrl_turn_v1}"
...
SFT_ROLLOUT_BATCH_SIZE="${SFT_ROLLOUT_BATCH_SIZE:-$(( (EXPECTED_CANONICAL_RECORDS + 1) / 2 * 2 ))}"
SFT_NUM_ROLLOUT="${SFT_NUM_ROLLOUT:-}"
SFT_SAVE_INTERVAL="${SFT_SAVE_INTERVAL:-100}"
SFT_CHECKPOINT_KEEP_LAST="${SFT_CHECKPOINT_KEEP_LAST:-4}"
...
PIPELINE_TOOLRL_APPLY_CHAT_TEMPLATE_KWARGS="${TOOLRL_APPLY_CHAT_TEMPLATE_KWARGS:-}"
if [[ -z "$PIPELINE_TOOLRL_APPLY_CHAT_TEMPLATE_KWARGS" ]]; then
  PIPELINE_TOOLRL_APPLY_CHAT_TEMPLATE_KWARGS='{"enable_thinking":false}'
fi
...
python - "$LIVE_DATA_ROOT/dataset_manifest.json" "$PREMATERIALIZED_RL_VIEW_ROOT/manifest.json" \
  "$EXPECTED_DATASET_VERSION" "$EXPECTED_CANONICAL_RECORDS" "$EXPECTED_EXCLUDED_RECORDS" \
  "$EXPECTED_TOOLRL_PROTOCOL" <<'PY'
import json, sys
...
TOOLRL_PROTOCOL=$EXPECTED_TOOLRL_PROTOCOL
...
TOOLRL_APPLY_CHAT_TEMPLATE_KWARGS=$PIPELINE_TOOLRL_APPLY_CHAT_TEMPLATE_KWARGS
TOOLRL_RBS=4
TOOLRL_N=4
TOOLRL_GBS=16
...
if [[ ! -f "$RUN_ROOT/sft.complete" ]]; then
  SFT_RESUME=()
  SFT_DURATION=(SFT_EPOCH_ONLY=1 NUM_EPOCH=1)
  [[ -f "$SFT_DIR/latest_checkpointed_iteration.txt" ]] && SFT_RESUME+=(RESUME_DIR="$SFT_DIR")
  if [[ -n "$SFT_NUM_ROLLOUT" ]]; then
    SFT_DURATION=(SFT_EPOCH_ONLY=0 NUM_ROLLOUT="$SFT_NUM_ROLLOUT")
  fi
  run_logged sft "${SFT_COMMON[@]}" "${SFT_RESUME[@]}" \
    PROMPT_DATA="$SFT_DATA" SAVE_DIR="$SFT_DIR" RUN_NAME="${RUN_ID}_sft" \
    "${SFT_DURATION[@]}" ROLLOUT_BATCH_SIZE="$SFT_ROLLOUT_BATCH_SIZE" \
    SAVE_INTERVAL="$SFT_SAVE_INTERVAL" CHECKPOINT_KEEP_LAST="$SFT_CHECKPOINT_KEEP_LAST" \
    bash drug_agent/scripts/run_qwen3_5_9b_drug_sft_full.sh
