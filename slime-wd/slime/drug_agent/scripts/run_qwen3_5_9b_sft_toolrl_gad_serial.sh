#!/usr/bin/env bash
# Formal single-worker pipeline for 373 canonical records on 8 x H200.
...
SERIAL_RUN_ID="${SERIAL_RUN_ID:-Qwen3.5-9B_current373_serial_$(date +%Y%m%d_%H%M%S)}"
...
if [[ "$SOURCE_COUNT" -ne 373 ]]; then
  echo "Expected 373 canonical records, found $SOURCE_COUNT in $CANONICAL_DATA" >&2
  exit 2
fi
...
if [[ ! -f "$RUN_ROOT/prepare_toolrl_data.complete" ]]; then
run_logged prepare_toolrl_data \
  python -m drug_agent.toolrl.convert_react_to_toolrl_steps \
    --input "$CANONICAL_DATA" --output "$TOOLRL_DATA" \
  ...
  echo "[serial] SKIP completed prepare_toolrl_data"
fi

if [[ ! -f "$RUN_ROOT/prepare_gad_data.complete" ]]; then
run_logged prepare_gad_data \
  ...
  echo "[serial] SKIP completed prepare_gad_data"
fi

TOOLRL_COUNT=$(wc -l < "$TOOLRL_DATA")
GAD_COUNT=$(wc -l < "$GAD_DATA")
if [[ "$TOOLRL_COUNT" -ne 3028 || "$GAD_COUNT" -ne 3234 ]]; then
  echo "Unexpected derived counts: ToolRL=$TOOLRL_COUNT GAD=$GAD_COUNT" >&2
  exit 2
fi

# SFT: TP4/PP2 is compatible with four KV groups and uses all eight GPUs.
# GBS=1 preserves all 373 records without padding or dropping the odd tail.
...
run_logged sft \
  env CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
    ...
    NUM_EPOCH=1 ROLLOUT_BATCH_SIZE=373 GLOBAL_BATCH_SIZE=1 MAX_TOKENS_PER_GPU=16384 \
...
# ToolRL official baseline: four samples per fixed history-only expert state.
...
run_logged toolrl \
  env CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
    ...
    NUM_GPUS=8 TENSOR_MODEL_PARALLEL_SIZE=4 PIPELINE_MODEL_PARALLEL_SIZE=2 \
    CONTEXT_PARALLEL_SIZE=1 EXPERT_MODEL_PARALLEL_SIZE=1 EXPERT_TENSOR_PARALLEL_SIZE=1 \
    ROLLOUT_NUM_GPUS_PER_ENGINE=1 ROLLOUT_BATCH_SIZE=4 N_SAMPLES_PER_PROMPT=4 \
    GLOBAL_BATCH_SIZE=16 NUM_ROLLOUT=757 NUM_EPOCH=1 \
    ROLLOUT_MAX_PROMPT_LEN=98304 ROLLOUT_MAX_RESPONSE_LEN=4096 ROLLOUT_MAX_CONTEXT_LEN=102400 \
    ROLLOUT_TEMPERATURE=1.0 SGLANG_MEM_FRACTION_STATIC=0.70 MAX_TOKENS_PER_GPU=8192 \
    LOG_PROBS_CHUNK_SIZE=2048 RECOMPUTE_LOSS_FUNCTION=1 \
    TOOLRL_REWARD_MODE=official LR=2e-7 SAVE_INTERVAL=200 RECOMPUTE_FULL=1 RECOMPUTE_NUM_LAYERS=1 \
...
# GAD Stage 2a starts from SFT (not ToolRL) and deterministically covers 3234
# states as 1617 batches of two.
...
run_logged gad_negative_generation \
  env CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
    ...
    MODEL_ARGS_FILE="$MODEL_ARGS_FILE" HF_CHECKPOINT="$HF_CHECKPOINT" REF_LOAD="$REF_LOAD" \
    NUM_GPUS=8 TENSOR_MODEL_PARALLEL_SIZE=4 ROLLOUT_NUM_GPUS_PER_ENGINE=1 \
    ROLLOUT_BATCH_SIZE=2 NUM_ROLLOUT=1617 \
    ROLLOUT_MAX_PROMPT_LEN=98304 ROLLOUT_MAX_RESPONSE_LEN=4096 ROLLOUT_MAX_CONTEXT_LEN=102400 \
    ROLLOUT_TEMPERATURE=0.8 SGLANG_MEM_FRACTION_STATIC=0.70 MAX_TOKENS_PER_GPU=8192 \
...
# Same-source 9B discriminator. Batch 1 and 2048 tokens leave headroom for
# full-parameter Adam and the paired positive/negative forward graphs.
...
run_logged gad_discriminator_warmup \
  env CUDA_VISIBLE_DEVICES=7 \
    ...
    DISCRIMINATOR_EPOCHS=1 DISCRIMINATOR_BATCH_SIZE=1 DISCRIMINATOR_LR=5e-7 \
    DISCRIMINATOR_MAX_LENGTH=2048 DISCRIMINATOR_CLIP_GRAD=0.2 DISCRIMINATOR_SAVE_INTERVAL=400 \
    DISCRIMINATOR_KEEP_LAST="$DISCRIMINATOR_KEEP_LAST" \
...
# Reserve GPU 7 for the online discriminator. The generator uses six cards as
# TP2/DP3; GPU 6 is intentionally left unused to isolate the service and avoid
# forcing an awkward seven-way data-parallel topology.
...
CUDA_VISIBLE_DEVICES=7 \
  ...
  DISCRIMINATOR_LR=5e-7 DISCRIMINATOR_MAX_LENGTH=2048 DISCRIMINATOR_UPDATE_STEPS=1 \
  DISCRIMINATOR_CLIP_GRAD=0.2 DISCRIMINATOR_REWARD_CLIP=2.0 DISCRIMINATOR_SAVE_INTERVAL=400 \
  DISCRIMINATOR_KEEP_LAST="$DISCRIMINATOR_KEEP_LAST" \
...
run_logged gad \
  env CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 \
    ...
    MODEL_ARGS_FILE="$MODEL_ARGS_FILE" HF_CHECKPOINT="$HF_CHECKPOINT" REF_LOAD="$REF_LOAD" \
    NUM_GPUS=6 TENSOR_MODEL_PARALLEL_SIZE=2 ROLLOUT_NUM_GPUS_PER_ENGINE=1 \
    ROLLOUT_BATCH_SIZE=2 N_SAMPLES_PER_PROMPT=3 GLOBAL_BATCH_SIZE=6 NUM_ROLLOUT=1617 \
    ROLLOUT_MAX_PROMPT_LEN=98304 ROLLOUT_MAX_RESPONSE_LEN=4096 ROLLOUT_MAX_CONTEXT_LEN=102400 \
    ROLLOUT_TEMPERATURE=0.8 SGLANG_MEM_FRACTION_STATIC=0.70 MAX_TOKENS_PER_GPU=8192 \
    LOG_PROBS_CHUNK_SIZE=2048 \
    STUDENT_LR=1e-7 KL_LOSS_COEF=0.001 SAVE_INTERVAL=200 \
