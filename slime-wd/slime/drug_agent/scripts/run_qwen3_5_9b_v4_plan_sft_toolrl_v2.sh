...
if [[ -n "$PREMATERIALIZED_RL_VIEW_ROOT" ]]; then
  ...
  TOOLRL_MANIFEST="$PREMATERIALIZED_RL_VIEW_ROOT/toolrl/context_manifest.json"
fi
...
for path in "$CANONICAL_DATA" "$TOOL_CATALOG" "$HF_CHECKPOINT" "$REF_LOAD/latest_checkpointed_iteration.txt" "$BASE_SFT_DIR/latest_checkpointed_iteration.txt" "$MODEL_ARGS_FILE"; do
  require_path "$path"
done
...
TOOLRL_COMMON=(
  ...
  HF_CHECKPOINT="$HF_CHECKPOINT" REF_LOAD="$REF_LOAD" NUM_GPUS=8
  ...
  ADVANTAGE_ESTIMATOR=grpo NORMALIZE_ADVANTAGES=0 ROLLOUT_MAX_PROMPT_LEN="$ROLLOUT_MAX_PROMPT_LEN"
  ...
  DISTRIBUTED_TIMEOUT_MINUTES=60 ADAM_BETA1=0.9 ADAM_BETA2=0.95 USE_ROLLOUT_LOGPROBS=0 USE_KL_LOSS=0
...
for tier in shortest p50 p95 near_limit; do
  ...
  if [[ ! -f "$RUN_ROOT/$stage.complete" ]]; then
    ...
    python -m drug_agent.scripts.check_rl_training_gate "$LOG_ROOT/$stage.log" 2 --minimum-nonzero-group-ratio 1.0
    mark_complete "$stage"
  fi
done
...
if [[ ! -f "$RUN_ROOT/toolrl_gate_near_limit_rbs4_offload.complete" ]]; then
  ...
  python -m drug_agent.scripts.check_rl_training_gate \
    ...
    --minimum-nonzero-group-ratio 1.0
...
if [[ ! -f "$RUN_ROOT/toolrl_multi_update.complete" ]]; then
  run_logged toolrl_multi_update "${TOOLRL_COMMON[@]}" PROMPT_DATA="$TOOLRL_DATA" \
    ...
    bash drug_agent/toolrl/scripts/run_toolrl_grpo.sh
  python -m drug_agent.scripts.check_rl_training_gate "$LOG_ROOT/toolrl_multi_update.log" 10 --minimum-nonzero-group-ratio 1.0
