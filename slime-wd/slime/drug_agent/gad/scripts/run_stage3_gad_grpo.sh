...
ray job submit --address=http://127.0.0.1:8265 --runtime-env-json="$RUNTIME_ENV" \
  ...
  "${MODEL_ARGS[@]}" \
  --hf-checkpoint "$HF_CHECKPOINT" --ref-load "$REF_LOAD" "${LOAD_ARGS[@]}" "${SAVE_EXTRA_ARGS[@]}" \
