...
def log_rollout_data(rollout_id, args, samples, rollout_extra_metrics, rollout_time) -> bool:
    configured_output = os.environ.get("GAD_TRAJECTORY_LOG")
    if configured_output:
        output = Path(configured_output)
    else:
        save_dir = getattr(args, "save", None)
        if not save_dir:
            raise ValueError("GAD_TRAJECTORY_LOG is required when checkpoint saving is disabled")
        output = Path(save_dir) / "gad_trajectories.jsonl"
