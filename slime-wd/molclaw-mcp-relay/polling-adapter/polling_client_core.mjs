...
export async function awaitToolResult({ request, name, arguments: args = {}, waitSeconds = 60,
  ...
  while (true) {
    ...
    const result = await request({ method:'tools/call', params:{ name, arguments:{...args,
      ...
      {signal, recoveryTimeoutMs:remaining, timeout:Math.min(remaining,(waitSeconds+60)*1000)});
    let payload;
