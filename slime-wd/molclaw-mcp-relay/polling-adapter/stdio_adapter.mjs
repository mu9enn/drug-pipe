...
log.write({event:'adapter_start',version:'recovery-20260919',headersTimeoutMs:Number(process.env.MOLCLAW_HEADERS_TIMEOUT_MS??14400000),pollSeconds:interval});
...
const server=createAdapter({recoveryBudgetMs,Server,schemas,remote,waitSeconds:interval,
  onEvent:e=>log.write(e)});
server.onclose=()=>{void remote.close();};
await server.connect(new StdioServerTransport());
log.write({event:'adapter_ready',version:'recovery-20260919'});
