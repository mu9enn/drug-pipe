...
P=pathlib.Path('/home/sunxiangyu/slime_sxy/group-space/sunxiangyu/drug-pipe');W=P/'slime-wd';M=W/'dsh-molbench/pretrained_matrix';N=P.parent/'drug_wd/drug_pipe_regular_v1_20260908/experiments/cpu_questions_0916a';NS='ailab-ma4agismall'
...
def command(item,role,attempt=1):
 e=item['env'];name=f"av4-cpu-{item['label']}-0916c-{role}{attempt}"
 base=f"infra-aligned-v4-9b-{item['label']}-cpu-0916c";infra=f'{base}-{role}{attempt}';modelinfra=f'{base}-gpu{attempt}'
 env={k:e[k] for k in ['MODEL_DIR','MODEL_ID','MODEL_NAME','WORKSPACE_VARIANT']};env['MODEL_DIR']=worker_path(env['MODEL_DIR'])
 env.update(SETTING_LABEL=item['label'],INFRA_NAME=infra,MODEL_INFRA_NAME=modelinfra,RUN_NAME=f"aligned-v4-9b-{item['label']}-cpu-0916a",SAMPLE_IDS_FILE=worker_path(N/(item['label']+'-ids.json')),CPU_FRAMEWORK='1',CPU_RUN_TOKEN=item['token'],GPU_COUNT='2',TP_SIZE='2',EVAL_SEED='42',EVAL_MAX_WORKERS='1',EVAL_RECOVERY='1',SUITES='ms1 ms2 ms3 mo-opt mo-edit',NCCL_IB_DISABLE='1',DISTRIBUTED_JOB='true',OWNER_UID=str(os.getuid()),OWNER_GID=str(os.getgid()))
...
def pair(item,slot,attempt):
 ...
 I=W/'outputs'/f"infra-aligned-v4-9b-{item['label']}-cpu-0916c-framework{attempt}"
 G.mkdir(parents=True,exist_ok=True);I.mkdir(parents=True,exist_ok=True)
 run=W/'outputs/dsh_molbench_evals'/f"aligned-v4-9b-{item['label']}-cpu-0916a"
 ...
 try:
  ...
  env=dict(os.environ);env.update(item['env']);env.update(DRUG_PROJECT=str(P),INFRA_NAME=I.name,MODEL_INFRA_NAME=G.name,RUN_NAME=f"aligned-v4-9b-{item['label']}-cpu-0916a",SAMPLE_IDS_FILE=str(N/(item['label']+'-ids.json')),SETTING_LABEL=item['label'],CPU_RUN_TOKEN=item['token'],MODEL_PORT=str(port),DSH_PORT=str(3080+slot))
  cpu=subprocess.Popen(['/usr/bin/python3','-u',str(M/'run_cpu_eval.py')],env=env,stdout=(I/'controller.log').open('w'),stderr=subprocess.STDOUT,start_new_session=True);children.append(cpu)
 ...
 with (N/f'queue{slot}.lock').open('a') as lock:
  ...
  try:
   for item in m['settings']:
    ...
    # Worker recovery is explicit and bounded. Trajectory budget exhaustion
    # returns normally and never reaches this branch.
    journal=[]
    for attempt in range(item.get('start_worker_attempt',1),4):
     if attempt>1:time.sleep([60,180][attempt-2])
     rc=pair(item,slot,attempt);journal.append({'attempt':attempt,'exit_code':rc});save(N/(item['label']+'-workers.json'),journal)
     if rc==0:break
     if rc!=67:raise RuntimeError('Non-retryable CPU worker failure')
    else:raise RuntimeError('Worker recovery exhausted')
    run=W/'outputs/dsh_molbench_evals'/f"aligned-v4-9b-{item['label']}-cpu-0916a"
    score=[str(W/'outputs/dsh_eval_runtime/mo-score-venv/bin/python'),str(W/'dsh-molbench/run_dsh_molbench.py'),'--score-only','--run-dir',str(run)]
    for suite in ['ms1','ms2','ms3','mo-opt','mo-edit']:score+=['--suite',suite]
    skill=P/'workdir-skills/molclaw-l1-workspace' if item['env']['WORKSPACE_VARIANT']=='l1-flat' else W/'outputs'/f"infra-aligned-v4-9b-{item['label']}-cpu-0916c-gpu{attempt}"/'workspace_template'
    score+=['--skill-source',str(skill)]
    with (N/(item['label']+'-score.log')).open('w') as f:
     subprocess.run(score,stdout=f,stderr=subprocess.STDOUT,check=True)
     subprocess.run([score[0],str(W/'dsh-molbench/score_format_sensitivity.py'),'--run-dir',str(run)],stdout=f,stderr=subprocess.STDOUT,check=True)
