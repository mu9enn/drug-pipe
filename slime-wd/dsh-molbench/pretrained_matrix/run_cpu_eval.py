...
try:
 ...
 (I/'route.json').write_text(json.dumps({'framework':'CPU workspace with native Bubblewrap','model':f'http://127.0.0.1:{model_port}/v1 over model-only SSH forward','mcp_proxy':proxy,'molclaw_relay_used':False,'recovery_policy':'infra_v2','trajectory_budget_seconds':14400,'trajectory_budget_retry':False},indent=2))
 ...
 cmd=[sys.executable,'-u',str(W/'dsh-molbench/run_dsh_molbench.py'),'--dsh-url',f'http://127.0.0.1:{dsh_port}','--recover-infra','--continuation-state-file',str(pathlib.Path(env['SAMPLE_IDS_FILE']).with_name(env['SETTING_LABEL']+'-continuations.json')),'--rollout-only','--sample-ids-file',env['SAMPLE_IDS_FILE'],'--run-dir',str(R),'--task-timeout-sec','14400','--max-workers','1','--model-provider','slime-local','--model-id',env['MODEL_ID'],'--agent-preset','molclaw-v8-eval','--skill-source',str(skill),'--required-mcp-tools','81','--required-skill-count',str(count),'--system-prompt-file',str(P/'data-pipe/pipeline/cleaning/prompts/qwen35_system.md'),'--tokenizer-config',env['MODEL_DIR']+'/tokenizer_config.json','--runtime-audit-file',str(G/'runtime_audit.json')]
 for suite in ['ms1','ms2','ms3','mo-opt','mo-edit']:cmd+=['--suite',suite]
 if (R/'run_manifest.json').exists():cmd+=['--resume']
