...
isolated directory for one task. The current directory is the workspace. Execute
the task in `question.json` or the user prompt; when `question.json` exists, it
is authoritative.
...
An L1 skill documents a tool but does not prove that the tool is deployed.
Current native MCP tool availability is the authority.
...
5. **Trace every reported value.** Numbers in `result.md` must come from tool
   output or verified source artifacts before reporting them.
...
7. **Use native tools only.** Do not print or simulate tool calls with XML, DSML,
...
## Phase 0 — Silent Triage, Guidance Loading, and Plan
...
3. Treat L1 directory names as a catalog. After selecting tools, read only their
   `.claude/skills/L1_tools/<tool-name>/SKILL.md` files. Use one dedicated `Read`
   call per selected L1 file; do not combine an L1 read in a shell command with
   L2/L3, `CLAUDE.md`, system/runtime, or other skill inspection.
...
4. When a benchmark exists, report an appropriate quantitative comparison and
...
- benchmark validation only when applicable.

Every requested item must be present. Tables should carry units and identifiers.
Every number must be traceable to a successful tool result or verified artifact.
...
The raw agent session is the per-call audit trail; `run_log.md` is a stage-level
...
- Read relevant L3/L2/L1 guidance directly from `.claude/skills/` without
  reporting each read.
...
Follow the JSON output contract specified by the task for your final answer. Return only that JSON object, with evidence grounded in observations.
